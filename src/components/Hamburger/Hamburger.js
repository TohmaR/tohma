...
function Hamburger(){
  ...
  const scrollOverflowDelay = 6950;
  ...
  useLayoutEffect(() => {
    if (!menuTimeline.current) {
      ...
      .to(".hamburger__nav--text", {
        y: 0,
        duration: 0.5,
        delay: -0.2,
        stagger: 0.08
      })
      .fromTo(".hamburger__logo>svg>g", { opacity: 0}, {opacity: 1, stagger: 0.1, delay: -0.8})

      
      .to(hamburgerTradRef.current, { duration: 0.4, delay: -0.3, opacity : 1, y: 0})
      .to(hamburgerSocialRef.current, { duration: 0.4, delay: -0.3, opacity : 1, y: 0, display: "block"})
    }
